...
type View = "Workspace"|"Genomics"|"Gene explorer"|"Variant explorer"|"Data sources"|"Architecture";
...
export function Workspace() {
 ...
 return <div className="shell">
  ...
  <aside ref={sidebar} id="research-navigation" inert={mobile&&!menu?true:undefined} role={mobile&&menu?"dialog":undefined} aria-modal={mobile&&menu?true:undefined} className={menu?"sidebar open":"sidebar"} aria-label="Workspace navigation">
   ...
   <div className="workspace-selector"><span className="workspace-avatar">PG</span><div>Research workspace<small>Public research · v0.3</small></div><Layers size={16}/></div>
   <nav>{([{name:"Workspace",icon:LayoutDashboard},{name:"Genomics",icon:Dna},{name:"Gene explorer",icon:GitBranch},{name:"Variant explorer",icon:Layers},{name:"Data sources",icon:Database},{name:"Architecture",icon:Network}] as const).map(item=><button key={item.name} onClick={()=>navigate(item.name)} aria-current={view===item.name?"page":undefined}><item.icon size={18}/>{item.name}{view===item.name&&<ChevronRight size={15}/>}</button>)}</nav>
