...
export function Research({initialGene=""}:{initialGene?:string}){
 ...
 useEffect(()=>{if(selectedDrug){const el=document.getElementById("research-drug-details");el?.focus({preventScroll:true});el?.scrollIntoView({block:"start"});}},[selectedDrug]);
 useEffect(()=>{if(selectedPathway){const el=document.getElementById("research-pathway-details");el?.focus({preventScroll:true});el?.scrollIntoView({block:"start"});}},[selectedPathway]);
 ...
 {drug&&<div id="research-drug-details" tabIndex={-1} className="record-detail" aria-label={drug.name+" research details"}><div className="section-title"><div><h3>{drug.name}</h3><p>{drug.id} · {drug.drug_class}</p></div><button className="icon-button" aria-label="Close drug details" onClick={()=>setSelectedDrug(null)}><X size={18}/></button></div>
