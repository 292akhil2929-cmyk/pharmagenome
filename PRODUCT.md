...
## Open Decisions
Neon PostgreSQL is connected privately through Vercel. Through Phase 9, genomic explorers, sequence tools, drug/pathway research associations, statistical inference, CellMiner NCI-60 response modeling and the real-data analytical dashboard are implemented. Phase 8's evidence-constrained GPT-6 Astra integration and unavailable state remain implemented; live generation is gated on server-side OpenAI API access. Phase 10 portfolio packaging, external model validation and clinical use remain outside the delivered scope.
