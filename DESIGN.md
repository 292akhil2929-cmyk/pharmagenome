...
## Phase 8 evidence explanation

Place optional explanation directly after the computed interpretation so it reads as a subordinate evidence docket rather than a chat product. Reuse the established bordered surface, forest ink and sage status colors. A narrow dark rail carries the BrainCircuit icon; the body pairs a compact model identifier with a plain heading, access state and link back to the underlying analysis. Generated claims use ruled rows and small monospace evidence-key tags.

Keep all model states explicit: checking access, unavailable, ready, generating, verified result and retryable failure. The unavailable state explains that the application result remains complete. A failed model-status check appears as the unavailable state with a “Retry model status” action; a generation failure preserves the analysis and exposes “Retry explanation.” A successful result separates its evidence-bound summary, supported findings and scope checks, then displays provider, generation time and evidence checksum. “View underlying analysis” returns focus and scroll position to the calculated result.
